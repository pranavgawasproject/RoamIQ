...
export default async function WorkspacesPage({
  ...
  const catalogKnown = catalogCount > 0;
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex-1 pt-28 sm:pt-32">
        ...
        <section className="border-b border-border bg-secondary/40 py-14 sm:py-20">
          <div className="mx-auto max-w-7xl px-5 sm:px-8">
            ...
            <h1 className="mt-3 font-serif text-4xl font-semibold tracking-tight text-balance sm:text-5xl">{catalogKnown ? `${headlineCount.toLocaleString()} workspaces & stays, live from the database.` : "Workspaces & stays, live from the database."}</h1>
      ...
      <WaitlistSticky
        source="workspaces-list-sticky"
        context={waitlistContext}
        heading="Leaving the workspace index without a shortlist?"
        description="This index is a common last page in a session. Email plus an optional city is enough. We only write when a listed price or Wi-Fi value exists — no invented numbers, no fabricated urgency."
      />
      <Footer />
    </div>
  );
