  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex-1 pt-28 sm:pt-32">
        ...
        <section className="border-b border-border bg-secondary/20 py-14 sm:py-20">
          <div className="mx-auto max-w-7xl px-5 sm:px-8">
            <div className="text-sm font-medium uppercase tracking-widest text-accent">
              Common questions
            </div>
            <h2 className="mt-3 font-serif text-3xl font-semibold tracking-tight sm:text-4xl">
              What RoamIQ actually lists
            </h2>
            <dl className="mt-8 grid gap-5 md:grid-cols-2">
              {faqJsonLd.mainEntity.map((qa) => (
                <div key={qa.name} className="rounded-2xl border border-border bg-card/70 p-6">
                  <dt className="font-serif text-lg font-semibold text-foreground">{qa.name}</dt>
                  <dd className="mt-2 text-sm leading-relaxed text-muted-foreground">{qa.acceptedAnswer.text}</dd>
                </div>
              ))}
            </dl>
          </div>
      ...
      <WaitlistSticky
        source="about-sticky"
        heading="Still deciding if RoamIQ is the right shortlist?"
        description="About is a short stop. Email plus an optional city is enough. We only write when a listed workspace already has a description, price, or photo."
      />
      <Footer />
    </div>
  );
