...
export async function getDestinationForListingCity(
  ...
  try {
    const { data, error } = await supabase
      .from("cities")
      .select("id, name, country, internet_mbps, wifi_speed_p90, cost_usd, avg_temp, air_quality, safety_score, visa_difficulty, walkability_score, coworking_desk_usd, one_bed_rent_usd")
      .ilike("name", cityName)
      .limit(8);
    ...
    return {
      ...
      coworking_desk_usd: asFiniteNumber(chosen.coworking_desk_usd),
      one_bed_rent_usd: asFiniteNumber(chosen.one_bed_rent_usd),
    };
  } catch {
    return null;
  }
