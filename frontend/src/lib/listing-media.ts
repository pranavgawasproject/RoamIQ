...
export function usefulListingUnits(units: string | null | undefined): string | null {
  ...
  if (cleaned === "per day" || cleaned === "per month") return cleaned;
  return null;
}
...
/**
 * Prefer a short company_title subtitle when it adds info beyond company_name.
 * Never invent titles.
 */
export function usefulListingTitle(
  ...
  if (!companyTitle || typeof companyTitle !== "string") return null;
  const title = companyTitle.replace(/\s+/g, " ").trim();
  if (title.length < 3 || title.length > 120) return null;
  const name = (companyName || "").replace(/\s+/g, " ").trim().toLowerCase();
  if (name && title.toLowerCase() === name) return null;
  if (/https?:\/\//i.test(title)) return null;
  return title;
}

/**
 * Inclusions text when present and useful. Never invent amenities.
 */
export function usefulListingInclusions(raw: string | null | undefined): string | null {
  if (!raw || typeof raw !== "string") return null;
  const cleaned = raw.replace(/\s+/g, " ").trim();
  if (cleaned.length < 8 || cleaned.length > 600) return null;
