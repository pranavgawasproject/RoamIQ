## 2026-09-19 — JSON-LD for listed plan and host

Track A: `/workspaces/[id]` LodgingBusiness/LocalBusiness JSON-LD and FAQPage now repeat `product_name` and `contact_name`/`contact_designation` only when those strings already pass usefulListingProductName / usefulListingContactPerson (same Plan / Contact lines on the page). Index ListItem schema does the same. Empty rows stay empty — no invented plans or hosts. Sitemap not resubmitted (same routes).

GSC 2026-08-20 to 2026-09-18 (`https://nomads-travel-indol.vercel.app/`): venue-name clicks continue (`cafe nenom`, `cafe nook`, `coliving athens`, branded `roam iq` 1 click / 34 impressions). Query `4g-travel-tool.vercel.app` FLAG only (11 impressions, position ~3.1); no redirect. Branded CTR capped until the custom domain is live (human-owned).
