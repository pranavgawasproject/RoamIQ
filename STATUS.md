## 2026-09-19 — workspaces listed-inclusions filter

Track A: `/workspaces?included=1` keeps cards whose stored `inclusions` already pass `usefulListingInclusions` (the same line already shown under price on the card). Empty / pending strings stay out — no invented inclusions. Sitemap not resubmitted (same route, new query only).
